...
#if defined(NDEBUG)
# error "eFranc cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x74a2df9096fe995e7db6114db766bb3b41cf3b15a7fc57cfac461d7ea175865d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eFranc: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "eFranc Signed Message:\n";
...
{
    // eFranc: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 4200000); // eFranc: 4.2mio blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // eFranc: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // eFranc: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // eFranc: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // eFranc: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x74a2df9096fe995e7db6114db766bb3b41cf3b15a7fc57cfac461d7ea175865d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "RF20WBLDN";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("873384710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517601240;
        ...
        assert(block.hashMerkleRoot == uint256("0x3761dc2ec15151087ff7da8982d58a6ca0ff0d511cf1548f005a9f706be89e3d"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // eFranc: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eFrancMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eFrancMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eFrancMiner(CWallet *pwallet)
{
    printf("eFrancMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("efranc-miner");
    ...
    try { loop {
        ...
        printf("Running eFrancMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eFrancMiner, pwallet));
}
